 ...
 * rather than regressing to the cold-default newMeta() — same
 * observability contract for every error path.
 */
...
describe("get_fragment — error envelope `_meta` preservation", () => {
	test("PathValidationError on warm index → _meta.index_status reflects warm + tokenizer present", async () => {
		...
		const r = await handleGetFragment(TRAVERSE_INPUT, FAKE_VAULT_ROOT, index);
